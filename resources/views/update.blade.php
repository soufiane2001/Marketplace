...
<section class="myprofile p-5">
    ...
    {{ $message }}
           ...
           <form  style="padding-left:10%;padding-right:10%" method="POST" action="{{route('edit') }}"
           enctype="multipart/form-data"
           >
           @csrf
           <input type="hidden" name="id" value="{{ $annonce->id}}">
           <input type="hidden" name="idf" value="{{ $annonce->photo[0]->id}}">
         
           <label for="pwd" class="mt-3">Title:</label>
            <input type="text" class="form-control w-50"  name="title"  value="{{ $annonce->Titre}}"  id="pwd">
                               
            <label for="email" class="mt-3">Script:</label>
            <textarea name="script"  class="form-control w-75"  id="" cols="10" rows="4" >
            {{ $annonce->script}}
            </textarea><br>
    
            <label for="pwd">prix:</label>
            <input type="number" class="form-control w-25" name="prix"  value="{{ $annonce->prix}}" placeholder="Enter price" >
            <div class="form-group mt-5">
      <input type="file" class="form-control-file border w-25" name="photo">
    </div> 
    <div class="form-group">
  <label for="sel1">Select Categorie:</label>
  <select class="form-control w-25" id="sel1" name="category" value="{{ $annonce->category}}">
